/**
 * Постобработка предсказаний YOLOv8-segment.
 * Код взят из шаблона курса (react.md, основан на Hyuto/yolov8-seg-onnxruntime-web)
 * без изменений в логике — это универсальная работа с тензорами модели,
 * специфики трёх наших классов внутри нет (она только в labels.json).
 *
 * Модель на входе даёт ДВА тензора:
 *   output0 — рамки + вероятности классов + коэффициенты маски (детект-слой)
 *   output1 — прототипы масок сегментации (seg-слой)
 * Их совместная постобработка (NMS + сборка маски) вынесена в отдельные
 * ONNX-графы nms-yolov8.onnx и mask-yolov8-seg.onnx ради скорости в браузере.
 */
...
/**
 * Детекция и сегментация одного изображения.
 * @param {HTMLImageElement} image изображение для анализа
 * @param {HTMLCanvasElement} canvas canvas для отрисовки
 * @param {Object} session { net, nms, mask } — три ONNX-сессии
 * @param {Number} topk максимум рамок на класс
...
export const detectImage = async (
  ...
  const mask_img = new ImageData(new Uint8ClampedArray(overlay.data), modelHeight, modelWidth);
